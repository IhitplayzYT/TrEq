...
fn main() -> Result<(), Box<dyn Error>>{
    ...
    let mut app = App::new(clargs.output_profiles_path);
    ...
    if let Some(strm) = stream{
        ...
        if clargs.stereo{
            ...
            std::thread::spawn(move || {process_stereo_audio(app_clone, lcons_clone, rcons_clone, sample_rate, channels,64);});
        }else{
            let cons_clone = cons.clone();
            std::thread::spawn(move || {process_mono_audio(app_clone, cons_clone, sample_rate, channels,64);});
        }
